...
@pytest.mark.asyncio
async def test_creates_details_for_a_new_company(session, scraped, describe):
    raw = (
        '{"company_url": "https://dk.linkedin.com/company/acme",'
        ' "company_logo": "https://logo.test/acme.png"}'
    )
    await ensure_company_details(session, scraped("1", raw=raw))
    session.commit()
...
@pytest.mark.asyncio
async def test_known_company_is_not_regenerated(session, scraped, describe):
    await ensure_company_details(session, scraped("1"))
    session.commit()
    await ensure_company_details(session, scraped("2"))
...
async def test_companies_match_by_linkedin_slug_despite_name_differences(
    session, scraped, describe
):
    raw = '{"company_url": "https://dk.linkedin.com/company/acme"}'
    await ensure_company_details(session, scraped("1", company="Acme", raw=raw))
    session.commit()

    # Same slug under a different name and subdomain: no second row.
    raw = '{"company_url": "https://www.linkedin.com/company/acme"}'
    await ensure_company_details(session, scraped("2", company="Acme ApS", raw=raw))
    session.commit()
...
@pytest.mark.asyncio
async def test_name_match_backfills_missing_linkedin_slug(session, scraped, describe):
    await ensure_company_details(session, scraped("1"))  # raw has no company_url
    session.commit()
    assert company_details(session).linkedin_slug is None

    raw = '{"company_url": "https://dk.linkedin.com/company/acme"}'
    await ensure_company_details(session, scraped("2", raw=raw))
    session.commit()

...
from jobwatch.pipeline.sync_companies import enrich_company, get_or_create_company
...
def describe(monkeypatch) -> list[str]:
    ...
    monkeypatch.setattr("jobwatch.llm.gemini.generate_company_description", fake)
    return calls


def company_details(session: Session) -> Company:
    return session.scalars(select(Company)).one()


def queued_company_ids(session: Session) -> list[int]:
    return sorted(task.company_id for task in queued_tasks(session, EnrichCompany))
...
def test_slug_match_returns_the_existing_row_despite_name_differences(session):
    first = get_or_create_company(session, name="Acme", linkedin_slug="acme")
    session.commit()

    # Same slug under a different name: no second row.
    second = get_or_create_company(session, name="Acme ApS", linkedin_slug="acme")
...
def test_never_matches_by_name_so_a_slugless_scrape_creates_a_duplicate(session):
    ...
    session.commit()
    second = get_or_create_company(session, name="Acme")
...
@pytest.mark.asyncio
async def test_enrich_company_generates_description(session, scene: Scene, describe):
    company = scene.company_details(name="Acme", description="")

    await enrich_company(session, company.id)
...
@pytest.mark.asyncio
async def test_enrich_company_does_not_redo_work_already_done(session, scene: Scene, describe):
